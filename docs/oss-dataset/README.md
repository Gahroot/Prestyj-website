...
The canonical home is **<https://prestyj.com/data>** — always-fresh URLs:
...
- **Per-statistic permalinks:** <https://prestyj.com/stat/{id}>

For a pinned, citable copy, use one of the reputable third-party dataset
repositories the dataset is published to (each mints a DOI or stable
identifier) — see [Distribution](#distribution-third-party-repositories).
...
## Distribution (third-party repositories)

The dataset is intentionally *not* mirrored onto more properties we own. The
canonical home stays <https://prestyj.com/data>; for redundancy, citation, and
discovery we publish copies to **reputable, independently-operated dataset
repositories** that host and curate the data themselves and point back to the
canonical landing page:

| Repository | DOI? | Role |
| --- | --- | --- |
| Zenodo (CERN) | ✅ | Permanent DOI; primary academic citation primitive |
| Figshare (Digital Science) | ✅ | Permanent DOI |
| Harvard Dataverse | ✅ | Recognized research-data repository; permanent DOI |
| Mendeley Data (Elsevier) | ✅ | DataCite-indexed; permanent DOI |
| Hugging Face Datasets | — | ML-community discovery |
| Kaggle Datasets | — | Data-science discovery |
| data.world | — | Open-data community |
| OpenML | — | ML benchmark catalog |

Submission-ready bundles for each are generated by
`npm run dataset:bundles` into `docs/oss-dataset/distributions/`, with a
step-by-step checklist in
[`distributions/SUBMISSION-CHECKLIST.md`](./distributions/SUBMISSION-CHECKLIST.md).
Researcher, journalist, and directory outreach drafts are generated by
`npm run dataset:pitch`.

We do **not** create more self-owned GitHub / Pages / npm mirrors of the
dataset — those add crawl footprint, not authority. The goal is to be cited by
real third parties, not to link to ourselves from more places we control.
