...
export const WORKFLOW_GUIDES: readonly WorkflowGuide[] = [
  guide("lead-response", {
    ...
    agentJob:
      "Reply to new leads, ask the first questions, and route each lead to the right person.",
  ...
  guide("missed-calls", {
    ...
    guardrail:
      "A person takes over for emergencies, upset callers, or anything outside the approved call reasons.",
  ...
  guide("estimate-followup", {
    ...
    guardrail:
      "A person handles discounts, scope changes, and objections outside the approved answers.",
  ...
  guide("sales-followup", {
    ...
    agentJob:
      "Send approved next-step messages and remind the owner when a personal reply is needed.",
  ...
  guide("review-requests", {
    ...
    agentJob:
      "Ask eligible customers for a review at the right time and stop after a clear response.",
    firstMove: "Define who is eligible and the event that should trigger the request.",
    guardrail:
      "A person handles unhappy customers and resolves the issue before any review request.",
  ...
  guide("ad-production", {
    ...
    guardrail:
      "A person approves every claim, final cut, and brand-sensitive choice before publishing.",
    successMetric: "Track approved ads produced and cost per tested angle for 30 days.",
  }),
  guide("ad-reporting", {
    problem:
      "Ad decisions slow down when results live in separate places or lack a clear comparison.",
  ...
  guide("listing-leads", {
    problem:
      "Listing inquiries cool quickly when the first reply does not answer the immediate need.",
    ...
    guardrail:
      "A licensed person handles advice, offers, fair-housing-sensitive topics, and unusual questions.",
  ...
  guide("open-house-followup", {
    ...
    agentJob:
      "Send a timely check-in, collect intent, and flag visitors ready for a personal call.",
