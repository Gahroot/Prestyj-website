...
- **You provide a "suggested audience" hint** (interests, behaviors, demographics) that Andromeda uses as a _starting_ prior, not a hard filter.
...
The shift: you're no longer telling Meta who to target. You're telling it where to deliver (geo) and giving it enough creative variety that _it_ figures out who to target. That's the entire Andromeda mental model in one sentence, and it's the inverse of how home services accounts were built.
...
Below is the angle inventory we use for HVAC, plumbing, and roofing campaigns. Read the headings as _families_, not as individual ads — each family produces 4–10 distinct creative variants.
...
This is the angle most accounts under-use. You don't need to _target_ a specific neighborhood through ad set settings; you put the neighborhood name on the screen in the first 2 seconds of the ad, and Andromeda matches it to nearby users.
...
Most home services accounts spend the right _total_ on creative — they just allocate 100% of it to 4–6 polished spots and 0% to volume. The right allocation is roughly 80% volume / 20% polish, and the cost-per-variant economics only work with a batch pipeline.
...
This is what running [done-for-you social media](/ai-content-department) on top of [batch video ads](/batch-video-ads) actually buys you — one production day, every output channel. Most contractors run two separate vendors (an ad agency and a social agency) and pay twice for the same footage. The Andromeda-era operational model collapses both into one.
...
3. **Stop refilming the same hero ad every quarter.** Refilm the _angle inventory_ (the 7 families above), not a single hero. Volume across families beats polish within one family every time.
...
If you want this running on your account — the shoot, the batch pipeline, the organic compound — start with [batch video ads](/batch-video-ads) for the paid creative engine, and layer in [done-for-you social media](/ai-content-department) if you want the same footage doing double duty across organic. Two vendors collapse into one, and the production line item finally earns its keep.

For the cross-industry framework this HVAC/plumbing/roofing analysis plugs into, see the [Batch Video Ads Complete Guide (2026)](/blog/batch-video-ads-complete-guide-2026) — it covers the underlying volume math, production pipeline, and cost models that the Andromeda response is built on top of.
