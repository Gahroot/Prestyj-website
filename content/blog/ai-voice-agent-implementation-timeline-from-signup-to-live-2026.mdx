...
> **Evidence status:** This draft is noindexed because its 40%-faster onboarding claim, 92% on-time rate, and missed-lead-per-day estimate are not backed by an approved source file. The day-by-day workflow is a planning template, not a measured industry benchmark.
...
**Direct answer:** Prestyj’s target implementation window for a standard done-for-you AI voice agent is **3–5 business days** when phone-system access, CRM credentials, approved scripts, business hours, routing rules, and FAQs are ready at kickoff. DIY and custom enterprise timelines vary with integrations and compliance review. No approved source currently supports the draft’s 40%-faster onboarding, 92% on-time, or missed-leads-per-day figures, so those claims must be removed or sourced before indexing.
