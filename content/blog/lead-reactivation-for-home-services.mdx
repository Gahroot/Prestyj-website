...
**TL;DR: Home-service lead reactivation costs $3–$12 per reactivated lead, against $50–$200 to acquire a new one. Multi-channel email + SMS + AI voice campaigns reactivate 8–15% of a dormant database, versus 0.5–2% for email alone. A 2,000–5,000 contact database runs $2,000–$10,000 for a 60–90 day campaign.**
...
The gap is structural, not a typo: the acquisition cost was already paid when the lead first entered the CRM, so a reactivation campaign only carries outreach cost. Where paid channels must buy attention from strangers, reactivation spends $0.01–$1.50 per touch (see the channel table above) to re-contact someone who already raised a hand.
...
Across these ranges every trade models out above 15x, and most land between 25x and 80x — because the denominator is outreach cost only. Your actual multiple depends on list hygiene, consent coverage, ticket size, and whether dispatch can absorb the booked appointments.
...
## What ROI Does a Database Reactivation Campaign Produce for a Home Service Business?
...
Each row is arithmetic on the assumptions in the same row (database size x reactivation rate x blended ticket), not an observed client outcome. Reactivation rate is the variable that moves the result most — halve it and the ROI halves with it.

---

## Modeled ROI Scenarios (Illustrative, Not Client Results)

The three scenarios below are **modeled from the rate and cost ranges in this guide**, not case studies from named clients. Treat them as a worked example of the arithmetic — swap in your own database size, ticket, and close rate before budgeting.
